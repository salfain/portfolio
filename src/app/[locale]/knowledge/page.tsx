  ...
  return (
    <>
      ...
      <Container className="pb-20">
        {/*
          Boundary skeleton berada DI SINI, di dalam halaman hub — bukan di
          `loading.tsx`. Berkas itu memasang Suspense di atas seluruh subtree
          `knowledge/`, termasuk rute detail yang memanggil `notFound()`;
          shell ter-flush duluan dan 404 terkirim sebagai 200. Lihat
          docs/phase-5/NOTES.md N1 dan app/route-boundaries.test.ts.
        */}
        <Suspense fallback={<CardGridSkeleton columns={2} cards={4} />}>
          <KnowledgeHubBody locale={locale} />
        </Suspense>
      </Container>
    </>
  )
...
async function KnowledgeHubBody({ locale }: { locale: Locale }) {
  const t = await getTranslations('knowledge')
