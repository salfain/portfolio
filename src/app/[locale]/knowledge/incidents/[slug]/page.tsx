...
export async function generateStaticParams() {
  const slugs = await getPublishedDocumentSlugs(typeForSegment('incidents'))
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return documentDetailMetadata(locale, 'incidents', slug)
}

export default async function KnowledgeIncidentsDetailPage({
  params,
}: PageProps) {
  const { locale, slug } = await params
  setRequestLocale(locale)

  return <DocumentDetail locale={locale} segment="incidents" slug={slug} />
}
